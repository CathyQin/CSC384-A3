...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        # The following code is used to find all the statisfying (using queen rule) point on the table
        # scope means the table is restricted by qi and qj
        scope = [qi, qj]
        satisfy = []
        # loop row and column
        for x in qi.domain():
            for y in qj.domain():
                # two constrains of queen
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
          ...
          # else:
          #   unassigned += 1
        return  self._lb <= assigned <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        # check if num of val in _required meet both the upper and lower bound
        def checkMeetReq(pair):
            satisify = 0
            for var, val in pair:
                if val in self._required:
                    satisify += 1
            return self._lb <= satisify <= self._ub

        def paritalCheck(pair):
            satisify = 0
            for var, val in pair:
                if val in self._required:
                    satisify += 1
            return satisify <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        return findvals(varsToAssign, [(var, val)], checkMeetReq, paritalCheck)
